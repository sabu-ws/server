...
def scan_route():
    if request.method == "POST" and session["scan"] == False:
        ...
        try_start = False
        if (
            request.files.getlist("fileInput")[0].filename == ""
            and request.files.getlist("folderInput")[0].filename == ""
        ):
        ...
        for file in req_folder:
            ...
            if file.filename != "" or len(file_reader) != 0:
                ...
                if (
                    len(split_in_folder) > 15
                    or "" in split_in_folder
                    or ".." in split_in_folder
                    or "%" in split_in_folder
                ):
                ...
                if control.control(file_reader, valid_extension):
                    ...
                    session["scan_resultat"].append(
                        f"The file '{str(filename)}' has not an authorized extension and it can't be scan"
                    )
        # Start scan
        if try_start:
