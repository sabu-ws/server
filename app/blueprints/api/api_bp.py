		...
		if "X-SABUAPITOKEN" in request.headers and "X-SABUHOSTNAME" in request.headers:
			...
			if get_device_token:
				...
				try:
					...
					log.info(f"Endpoint {str(get_device_token.hostname)} has expire signature")
...
def join(*args, **kwargs):
	...
	log.info(f"Endpoint {str(device.hostname)} is connected and join a room {str(request.remote_addr)}")
	device.state = 1
	db.session.commit()
