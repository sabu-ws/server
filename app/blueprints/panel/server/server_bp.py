...
def connect_chart_cpu():
    ...
    get_cpu_metrics = Metrics.query.filter(
        Metrics.idDevice == int(get_server_device.id),
        Metrics.name == "cpu",
        Metrics.timestamp_ht >= delta_hours,
    ).all()
...
def connect_chart_cpu():
    ...
    get_server_device = Devices.query.filter_by(token="server").first()
    delta_hours = datetime.datetime.now() - datetime.timedelta(hours=24)
    get_ram_metrics = Metrics.query.filter(
        Metrics.idDevice == int(get_server_device.id),
        Metrics.name == "ram",
        Metrics.timestamp_ht >= delta_hours,
    ).all()
...
def connect_chart_net():
    ...
    get_server_device = Devices.query.filter_by(token="server").first()
    delta_hours = datetime.datetime.now() - datetime.timedelta(hours=24)
    get_netin_metrics = Metrics.query.filter(
        Metrics.idDevice == int(get_server_device.id),
        Metrics.name == "netin",
        Metrics.timestamp_ht >= delta_hours,
    ).all()
    get_netout_metrics = Metrics.query.filter(
        Metrics.idDevice == int(get_server_device.id),
        Metrics.name == "netout",
        Metrics.timestamp_ht >= delta_hours,
    ).all()
...
def netiface(iface=""):
    ...
    process = (
        subprocess.Popen(command, stdout=subprocess.PIPE).communicate()[0].decode()
    )
...
def services():
    ...
    for service in services:
        script_service_exec = (
            subprocess.Popen([script_service_path, service], stdout=subprocess.PIPE)
            .communicate()[0]
            .decode()
        )
        if "ago" in script_service_exec:
            ...
            runtime_svc.append("-")
    return render_template(
        "ap_srv_services.html", services=zip(services, status_svc, runtime_svc)
    )
