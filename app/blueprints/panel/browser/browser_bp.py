...
def index(MasterListDir=""):
	...
	if os.path.isdir(joining):
		...
		for i in list_items[1]:
			...
			creation_date = str(datetime.datetime.fromtimestamp(os.lstat(j).st_ctime)).split(".")[0]
			modification_date = str(datetime.datetime.fromtimestamp(os.lstat(j).st_mtime)).split(".")[0]
	...
	return render_template("ap_browser.html",items_file=items_file,items_dir=items_dir,cur_dir=cur_dir)
