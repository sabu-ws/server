				...
				if queryJob != None and queryJob != "Administrator":
					if AddUserForm.validate(form):
						if (
							...
							log.info(f"New user added {str(data['username'])} by {str(current_user.username)}")
							return "ok"
						else:
	...
	if "uuid" in request.form:
		...
		if queryUser is not None:
			if (
				...
				db.session.flush()
				db.session.commit()
				flash(f"The informations of {queryUser.username} has been change", "good")
				log.info(f"The informations of user {str(queryUser.username)} has been change by {str(current_user.username)}")
				return "ok"
			else:
				keys = list(dict(form.errors.items()))
				return jsonify(dict(form.errors.items())[keys[0]][0])
	...
	if "uuid" in request.form:
		...
		if user is not None:
			if user.OTPSecret is not None:
				...
				log.info(f"Totp of user {str(user.username)} has been deleted by {str(current_user.username)}")
				return "ok"
			else:
				return force_logout_user()
		else:
...
def del_user():
	if "uuid" in request.form:
		...
		if search is not None:
			...
			log.info(f"The user {search.username} has been deleted by {str(current_user.username)}")
			return "ok"
		else:
			return force_logout_user()
	else:
	...
	if "uuid" in request.form:
		...
		if search is not None:
			if search.enable == 1:
				...
				log.info(f"The user {str(search.username)} has been disable by {str(current_user.username)}")
			elif search.enable == 0:
				search.enable = 1
				flash(f"The user {search.username} has been enable", "good")
				log.info(f"The user {str(search.username)} has been enable by {str(current_user.username)}")
			db.session.commit()
			return "ok"
		else:
			return force_logout_user()
